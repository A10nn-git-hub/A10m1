...
class SuburbanHouseBuilder {
    ...
    createHouse(posX, posZ, rotationY = 0, styleIndex = 0) {
        ...
        const doorMesh = new THREE.Mesh(new THREE.BoxGeometry(doorWidth * 0.95, doorHeight, 0.08), this.materials.woodDoor);
        ...
        // Физический барьер проема двери жилого дома (блокирует проход до открытия)
        const barrierCenter = new THREE.Vector3(
            posX + sinR * (-houseD / 2),
            doorHeight / 2,
            posZ + cosR * (-houseD / 2)
        );
        const barrierBody = this.createStaticBox(barrierCenter.x, barrierCenter.y, barrierCenter.z, doorWidth * 0.95, doorHeight, 0.25, cx, cz);
        barrierBody.quaternion.setFromAxisAngle(new CANNON.Vec3(0, 1, 0), rotationY);

        this.doors.push({
            pivot: doorPivot,
            pos: doorWorldPos,
            barrier: barrierBody,
            barrierPos: { x: barrierCenter.x, y: barrierCenter.y, z: barrierCenter.z },
            targetAngle: 0,
            currentAngle: 0,
    ...
    update(deltaTime, playerPosition) {
        ...
        for (let i = 0; i < this.doors.length; i++) {
            ...
            // Физический барьер проема: закрыто -> блокирует вход, открыто (>65%) -> проход открыт
            if (door.barrier) {
...
window.SuburbanHouseBuilder = SuburbanHouseBuilder;
