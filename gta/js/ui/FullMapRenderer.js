            ...
            render() {
                ...
                for (let r = 0; r < gridRows; r++) {
                    for (let c = 0; c < gridCols; c++) {
                        ...
                        // Однородный естественный стиль районов города (без деления на активные и неактивные зоны)
                ...
                // 7. Скалы / валуны
                if (veg && veg.rockPositions) {
                    ctx.fillStyle = '#94a3b8';
                    for (const rk of veg.rockPositions) {
                        const rp = this.worldToMap(rk.x, rk.z);
                        ctx.beginPath();
                        ctx.arc(rp.x, rp.y, 1.8, 0, Math.PI * 2);
                        ctx.fill();
                    }
                }

                // 8. Автомобили трафика и шоукейса
                ...
                ctx.fillText('ВЫ ЗДЕСЬ', pp.x + 12, pp.y + 4);
            }
        }
