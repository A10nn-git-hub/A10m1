...
- `br_2d` - `Выживание` в стиле Standoff 2 с выбором игровых режимов хостом:
  ...
  - **Дуэль 1v1** (`duel_1v1`): дуэль до 2 игроков. Пустые места автоматически заполняются ботами (до 1 на каждую команду).
  - **Дуэль 2v2** (`duel_2v2`): дуэль до 4 игроков. Пустые места автоматически заполняются ботами (до 2 на каждую команду).

  Игроки и боты распределяются по двум командам (Команда 1 и Команда 2). Отрисовка бойцов использует динамическое окрашивание: члены вашей команды подсвечиваются сине-голубым цветом (`#32ade6`), противники — ярко-красным (`#ff453a`).
